...
public class RemoveGroupMemberCommand extends GroupUserCommand implements Command {
    final private Logger log = Logger.getLogger(RemoveGroupMemberCommand.class);


    public RemoveGroupMemberCommand(SendBotMessageService sendBotMessageService, GroupService groupService,
    ...
    private void removeUserFromGroup(Group group, BotUser user) {
        String message;
        try {
            ...
            message = String.format("Can't find user <b>%s</b> in group <b>%s</b>.",
                    user.getUsername(), group.getGroupName());
            sendBotMessageService.sendMessage(chatId, message);
            log.info(e);
        }
